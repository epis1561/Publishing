...
if(!sql_query(" DESCRIBE {$g5['new_win_table']} ", false)) {
    if(sql_query(" DESCRIBE {$g5['g5_shop_new_win_table']} ", false)) {
       ...
       $query_cp = sql_query(" CREATE TABLE IF NOT EXISTS `{$g5['new_win_table']}` (
					  ...
					  `nw_type` varchar(11) NOT NULL DEFAULT '0',
...
$sql = "select * $sql_common order by nw_2 desc, nw_1 asc, nw_id desc ";
...
<div class="tbl_head01 tbl_wrap">
    <form name="f" method="post" action="newwinlistupdate.php">
    <table>
    ...
    <tr class="<?php echo $bg; ?>">
        <input type="hidden" name="nw_id[]" value="<?= $row['nw_id'] ?>">
        <td class="td_num"><?php echo $row['nw_id']; ?></td>
        <td><?php echo $row['nw_subject']; ?></td>
        <td class="td_mngsmall"><?= $row['nw_content'] ?></td>
        <td class="td_mngsmall"><input type="text" name="nw_1[]" value="<?= $row['nw_1'] ?>"></td>
        <td class="td_mngsmall"><input type="checkbox" name="nw_2[<?= $row['nw_id'] ?>]" value="1" <?php if ($row['nw_2'] == "1") echo "checked"; ?>></td>
        <!--td class="td_mngsmall">
            <select name="nw_3[]">
                <option value="0">전부</option>
                <option value="1" <?php if ($row['nw_3'] == "1") echo "selected"; ?>>인트로</option>
                <option value="2" <?php if ($row['nw_3'] == "2") echo "selected"; ?>>메인</option>
            </select>
        </td>
...
<div class="btn_add01 btn_add">
        <a href="javascript:f.submit();">일괄수정</a>
    </div>    
