...
if(!sql_query(" DESCRIBE {$g5['menu_table']} ", false)) {
    sql_query(" CREATE TABLE IF NOT EXISTS `{$g5['menu_table']}` (
				  ...
				  `pageNum` int(11) NOT NULL DEFAULT '',				  
                  `me_name` varchar(255) NOT NULL DEFAULT '',
                  `me_link` varchar(255) NOT NULL DEFAULT '',
                  `me_target` varchar(255) NOT NULL DEFAULT '0',
				  `me_link` varchar(255) NOT NULL DEFAULT '',
				  `me_title_e` varchar(255) NOT NULL DEFAULT '',
				  `me_title01` varchar(255) NOT NULL DEFAULT '',
				  `me_title_b` varchar(255) NOT NULL DEFAULT '',
				  `me_title02` varchar(255) NOT NULL DEFAULT '',
...
<div id="menulist" class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col">새창</th>
        <th scope="col">순서</th>
        <th scope="col">pageTitle_e (왼쪽영문1)</th>
		<th scope="col">pageTitle01 (왼쪽영문_객실,스페셜만 이용)</th>
		<th scope="col">title_b (왼쪽영문2)</th>
		<th scope="col">pageTitle02 (오른쪽 한글)</th>
        <th scope="col" style="display:none;">PC사용</th>
        <th scope="col" style="display:none;">모바일사용</th>
        <th scope="col">관리</th>
    </tr>
    </thead>
    <tbody>
    ...
    <tr class="<?php echo $bg; ?> menu_list menu_group_<?php echo substr($row['me_code'], 0, 2); ?>">
		...
		<td class="td_num">
			<div <?php if(strlen($row['me_code']) == 2) { ?>style="display:none"<?php } ?>>			
				<label for="pageNum_<?php echo $i; ?>" class="sound_only">pageNum</label>
				<input type="text" name="pageNum[]" value="<?php echo $row['pageNum'] ?>" id="pageNum_<?php echo $i; ?>" class="frm_input" size="5">
			</div>
        </td>
        <td class="td_mng">
        ...
        <td class="td_itopt">
			<div <?php if(strlen($row['me_code']) == 2) { ?>style="display:none"<?php } ?>>
				<label for="me_title_e<?php echo $i; ?>" class="sound_only">me_title_e<strong class="sound_only"> 필수</strong></label>
				<input type="text" name="me_title_e[]" value="<?php echo $row['me_title_e'] ?>" id="me_title_e<?php echo $i; ?>" class="frm_input full_input">
			</div>
        </td>
		<td class="td_odrnum">
			<div <?php if(strlen($row['me_code']) == 2) { ?>style="display:none"<?php } ?>>
				<label for="me_title01<?php echo $i; ?>" class="sound_only">me_title01<strong class="sound_only"> 필수</strong></label>
				<input type="text" name="me_title01[]" value="<?php echo $row['me_title01'] ?>" id="me_title01<?php echo $i; ?>" class="frm_input full_input">
			</div>
        </td>
		<td class="td_itopt">
			<div <?php if(strlen($row['me_code']) == 2) { ?>style="display:none"<?php } ?>>
				<label for="me_title_b<?php echo $i; ?>" class="sound_only">me_title_b<strong class="sound_only"> 필수</strong></label>
				<input type="text" name="me_title_b[]" value="<?php echo $row['me_title_b'] ?>" id="me_title_b<?php echo $i; ?>" class="frm_input full_input">
			</div>
        </td>
		<td class="td_itopt">
			<div <?php if(strlen($row['me_code']) == 2) { ?>style="display:none"<?php } ?>>
				<label for="me_title02<?php echo $i; ?>" class="sound_only">me_title02<strong class="sound_only"> 필수</strong></label>
				<input type="text" name="me_title02[]" value="<?php echo $row['me_title02'] ?>" id="me_title02<?php echo $i; ?>" class="frm_input full_input">
			</div>
        </td>
